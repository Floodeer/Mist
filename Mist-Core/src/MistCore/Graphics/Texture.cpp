...
namespace Mist {
	...
	{
		return std::make_shared<Texture2D>(specification);
	}
		...
		: m_Path(path)
	{
		...
		if (data)
		{
			...
			MCORE_ASSERT(internalFormat & dataFormat, "Format not supported!");
	...
	void Texture2D::SetData(void* data, uint32_t size)
	{
		...
		MCORE_ASSERT(size == m_Width * m_Height * bpp, "Data must be entire texture!");
